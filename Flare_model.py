...
def flare_model(t,tpeak, fwhm, ampl, upsample=False, uptime=10):
    ...
    A,B,C,D1,D2,f1 = [0.9687734504375167,-0.251299705922117,0.22675974948468916,0.15551880775110513,1.2150539528490194,0.12695865022878844]
    ...
    if upsample:
        ...
        flareup = ((1/2) * np.sqrt(np.pi) * A * C * f1* np.exp(-D1*timeup+((B/C)+(D1*C/2))**2)*special.erfc(((B-timeup)/C)+ (C*D1/2)))
        +((1/2) * np.sqrt(np.pi) * A * C * f2* np.exp(-D2*timeup+((B/C)+(D2*C/2))**2)*special.erfc(((B-timeup)/C) + (C*D2/2)))
        ...
        flare = ((1/2) * np.sqrt(np.pi) * A * C * f1* np.exp(-D1*t_new+((B/C)+(D1*C/2))**2)*
                 special.erfc(((B-t_new)/C)+ (C*D1/2)))+((1/2) * np.sqrt(np.pi) * A * C * f2*
                                                         np.exp(-D2*t_new+((B/C)+(D2*C/2))**2)*special.erfc(((B-t_new)/C) + (C*D2/2)))
